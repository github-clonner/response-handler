...
class Response {
  ...
  * antes de aplicar el formato. En este punto podemos ver el contenido original de la respuesta.
  ...
  */
  static error (options = {}) {
    ...
    const onError = options.onError || ((result) => { if (result.code === 500) { console.log(result) } })
...
function normalizeSuccess (data) {
  if (data && data.name && data.name === 'InsacResponseSuccess') { return data }
...
function normalizeError (err) {
  if (err && err.name && err.name === 'InsacResponseError') { return err }
